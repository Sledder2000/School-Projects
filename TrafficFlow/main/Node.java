...
public class Node<T extends Comparable<T>> implements Comparable<Node<T>> {
    ...
    public void removeEdge(Node<T> otherNode) {
        _edges.remove(otherNode._data.hashCode(), otherNode);
    }

    public boolean hasEdge(Node<T> otherNode) {
    ...
    public boolean isUGraph() {
        Integer myHash = _data.hashCode();
        for (Node<T> n : _edges.values()) {
            if (!n._edges.containsKey(myHash)) {
                return false;
            }
        }
    return true;
    ...
    public void hasCycle(Node startNode, int maxRec) {
        if (maxRec > 20) {
            return;
        }
        this._state += 1;
        for (Node<T> node : _edges.values()) {
            hasCycle(startNode, maxRec += 1);
        }
    }

    public boolean connected(Node<T> node) {
        if (_state == 1) {
            return true;
        }
        ...
        q.add(node);
        _state = 1;
            for(Node<T> n : q.remove().getEdges().values()) {
                if (n.getState() == 1) {
                    return true;
                }
                n.setState(1);
